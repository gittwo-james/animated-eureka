...
const (
    r2UploadStatusInitiated = "initiated"
    r2UploadStatusCompleted = "completed"
    r2UploadStatusFailed    = "failed"
)

type R2FileOpsHandler struct {
    DB       *gorm.DB
...
func (h *R2FileOpsHandler) Register(r *gin.Engine) {
    g := r.Group("/files")
...
func (h *R2FileOpsHandler) InitiateUpload(c *gin.Context) {
    if h.DB == nil {
    ...
    fileID, err := h.ensureFileForUpload(c, uid, orgID, role, req.FileID, name, contentType, req.Size)
    if err != nil {
        return
    }
    ...
    session := models.FileUploadSession{
        ...
        Status:         r2UploadStatusInitiated,
    }
    ...
    if multipart {
        ...
        if err != nil {
            h.logError("r2 create multipart upload failed", err)
        ...
        for pn := int32(1); pn <= maxPart; pn++ {
            ...
            if err != nil {
                h.logError("r2 presign upload part failed", err)
        ...
        h.audit(c, &uid, "file_upload_initiated", "file", fileID, map[string]interface{}{"upload_session_id": sessionID.String(), "multipart": true, "total_parts": totalParts, "part_size": partSizeBytes})
    ...
    if err != nil {
        h.logError("r2 presign put failed", err)
        c.JSON(http.StatusServiceUnavailable, gin.H{"error": "failed to presign upload"})
        return
    }

    h.audit(c, &uid, "file_upload_initiated", "file", fileID, map[string]interface{}{"upload_session_id": sessionID.String(), "multipart": false})
...
func (h *R2FileOpsHandler) presignMultipartParts(c *gin.Context, uid uuid.UUID, orgID uuid.UUID, req initiateUploadRequest) {
    ...
    if session.Status != r2UploadStatusInitiated {
...
func (h *R2FileOpsHandler) CompleteUpload(c *gin.Context) {
    if h.DB == nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "database not configured"})
        return
    }
    ...
    if session.IsMultipart {
        ...
        if err := h.R2.CompleteMultipartUpload(ctx, session.StoragePathR2, *session.R2UploadID, parts); err != nil {
            h.logError("r2 complete multipart upload failed", err)
            msg := err.Error()
            _ = h.DB.Model(&models.FileUploadSession{}).Where("id = ?", session.ID).Updates(map[string]interface{}{"status": r2UploadStatusFailed, "last_error": msg}).Error
        ...
        if _, err := h.R2.HeadObject(ctx, session.StoragePathR2); err != nil {
            h.logError("r2 head object failed", err)
    ...
    if err != nil {
        h.logError("failed to compute object hash", err)
    ...
    if expSha != strings.ToLower(shaHex) {
        h.audit(c, &uid, "file_hash_mismatch", "file", session.FileID, map[string]interface{}{"kind": "sha256", "expected": expSha, "actual": shaHex})
        _ = h.R2.DeleteObject(ctx, session.StoragePathR2)
        msg := "sha256 mismatch"
        _ = h.DB.Model(&models.FileUploadSession{}).Where("id = ?", session.ID).Updates(map[string]interface{}{"status": r2UploadStatusFailed, "last_error": msg}).Error
    ...
    if expMd5 != "" && expMd5 != strings.ToLower(md5Hex) {
        h.audit(c, &uid, "file_hash_mismatch", "file", session.FileID, map[string]interface{}{"kind": "md5", "expected": expMd5, "actual": md5Hex})
        _ = h.R2.DeleteObject(ctx, session.StoragePathR2)
        msg := "md5 mismatch"
        _ = h.DB.Model(&models.FileUploadSession{}).Where("id = ?", session.ID).Updates(map[string]interface{}{"status": r2UploadStatusFailed, "last_error": msg}).Error
    ...
    for _, key := range toDelete {
        if err := h.R2.DeleteObject(ctx, key); err != nil {
            h.logError("failed to delete old version object", err)
        }
    }

    h.audit(c, &uid, "file_upload_completed", "file", session.FileID, map[string]interface{}{"upload_session_id": session.ID.String(), "version_id": version.ID.String(), "version_number": newVer})
...
func (h *R2FileOpsHandler) ListFiles(c *gin.Context) {
    if h.DB == nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "database not configured"})
        return
    }
    ...
    q := h.DB.Where("owner_id = ? AND organization_id = ?", uid, orgID).Order("created_at DESC").Limit(limit).Offset(offset)
    ...
    var total int64
    _ = h.DB.Model(&models.File{}).Where("owner_id = ? AND organization_id = ?", uid, orgID).Count(&total).Error

    c.JSON(http.StatusOK, gin.H{"data": files, "total": total, "limit": limit, "offset": offset})
}

func (h *R2FileOpsHandler) DeleteFile(c *gin.Context) {
    if h.DB == nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "database not configured"})
        return
    }
...
func (h *R2FileOpsHandler) Download(c *gin.Context) {
    ...
    if err != nil {
        h.logError("r2 presign get failed", err)
        c.JSON(http.StatusServiceUnavailable, gin.H{"error": "failed to generate download url"})
        return
    }

    h.audit(c, &uid, "file_download", "file", file.ID, map[string]interface{}{"version_id": version.ID.String(), "version_number": version.VersionNumber})
...
func (h *R2FileOpsHandler) ListVersions(c *gin.Context) {
    ...
    c.JSON(http.StatusOK, gin.H{"file_id": file.ID.String(), "versions": versions})
}

func (h *R2FileOpsHandler) RestoreVersion(c *gin.Context) {
    if h.DB == nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "database not configured"})
        return
    }
    ...
    if err := h.R2.CopyObject(c.Request.Context(), version.StoragePathR2, newKey, file.FileType); err != nil {
        h.logError("r2 copy object failed", err)
    ...
    if err := tx.Model(&models.File{}).Where("id = ?", file.ID).Updates(map[string]interface{}{"storage_path_r2": newKey, "size": version.FileSize}).Error; err != nil {
        tx.Rollback()
        c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to update file"})
        return
    }
    ...
    h.audit(c, &uid, "file_version_restored", "file", file.ID, map[string]interface{}{"restored_from_version_id": version.ID.String(), "new_version_id": newVersion.ID.String(), "new_version_number": newVerNum})
...
func (h *R2FileOpsHandler) VerifyHash(c *gin.Context) {
    if h.DB == nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "database not configured"})
        return
    }
    ...
    if okSha && okMd5 && okSize {
        h.audit(c, &uid, "file_hash_verified", "file", file.ID, map[string]interface{}{"version_id": version.ID.String(), "result": "ok"})
        c.JSON(http.StatusOK, gin.H{"ok": true, "version_id": version.ID.String(), "sha256_hash": shaHex, "md5_hash": md5Hex, "size": size})
        return
    }

    h.audit(c, &uid, "file_hash_mismatch", "file", file.ID, map[string]interface{}{"kind": "integrity", "stored_sha256": version.Sha256Hash, "stored_md5": version.Md5Hash, "stored_size": version.FileSize, "actual_sha256": shaHex, "actual_md5": md5Hex, "actual_size": size})
...
func (h *R2FileOpsHandler) ensureFileForUpload(c *gin.Context, uid uuid.UUID, orgID uuid.UUID, role string, fileIDRaw string, name string, contentType string, size int64) (uuid.UUID, error) {
    if strings.TrimSpace(fileIDRaw) != "" {
        ...
        if err := h.DB.Model(&models.File{}).Where("id = ?", file.ID).Updates(map[string]interface{}{"name": name, "file_type": contentType, "size": size}).Error; err != nil {
...
func (h *R2FileOpsHandler) computeObjectHashes(ctx context.Context, storageKey string) (string, string, int64, error) {
...
func (h *R2FileOpsHandler) enforceMaxVersions(tx *gorm.DB, fileID uuid.UUID) ([]string, error) {
...
func (h *R2FileOpsHandler) currentIdentity(c *gin.Context) (uuid.UUID, uuid.UUID, string, bool) {
    uidStr, ok := c.Get(middleware.ContextUserID)
    if !ok {
        c.JSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
        return uuid.Nil, uuid.Nil, "", false
    }
    orgStr, ok := c.Get(middleware.ContextOrganizationID)
    if !ok {
        c.JSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
        return uuid.Nil, uuid.Nil, "", false
    }
    roleAny, _ := c.Get(middleware.ContextRole)
    ...
    orgID, err := uuid.Parse(fmt.Sprintf("%v", orgStr))
    if err != nil {
        c.JSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
        return uuid.Nil, uuid.Nil, "", false
    }
...
func (h *R2FileOpsHandler) canReadFile(uid uuid.UUID, role string, file models.File) bool {
    if file.OwnerID == uid {
        return true
    }
    if role == "admin" {
    ...
    ok, err := permRepo.CheckUserCanAccessFile(uid, file.ID, "read")
    return err == nil && ok
}

func (h *R2FileOpsHandler) canWriteFile(uid uuid.UUID, role string, file models.File) bool {
    if file.OwnerID == uid {
        return true
    }
    if role == "admin" {
    ...
    permRepo := repositories.NewPermissionRepository(h.DB)
    ok, err := permRepo.CheckUserCanAccessFile(uid, file.ID, "write")
    return err == nil && ok
}

func (h *R2FileOpsHandler) allowedContentType(contentType string) bool {
...
func (h *R2FileOpsHandler) audit(c *gin.Context, userID *uuid.UUID, action string, resourceType string, resourceID uuid.UUID, metadata map[string]interface{}) {
    if h.Audit == nil {
        return
    }
    if err := h.Audit.Log(userID, action, resourceType, resourceID, middleware.ExtractIPFromRequest(c), c.Request.UserAgent(), metadata); err != nil {
        h.logError("failed to write audit log", err)
    }
}

func (h *R2FileOpsHandler) logError(msg string, err error) {
    if h.Log != nil {
